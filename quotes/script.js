...
function getQuote( reset )
{
	...
	// insert a <br> into longer quotes
	if ( quote.length >= 50 && quote.length <= 100 && window.innerWidth > 900 )
	{
		for ( var j=(quote.length/2)-8; j < (quote.length/2)+8; j++ )
		{
			if ( quote.charAt(j) == ' ' )
			{
				quote = quote.substring(0,j) + "<br>" + quote.substring(j+1) ;
				break;
			}
		}
	}
    ...
    // Twitter handles texts that are longer than 140 chars automatically by refusing to send them, so we leave the task of shortening it to the user.
	document.getElementById("twitter").href = "https://twitter.com/intent/tweet?text=" + encodeURIComponent("\"" + tQuote + "\"") + encodeURIComponent("—"+author);
}
